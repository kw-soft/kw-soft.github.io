...
Container workloads are also exposed: the kernel page cache is shared between containers and the host, so any container with access to `AF_KEY`, `XFRM` netlink, or `AF_RXRPC` sockets: which is the default for unconstrained Docker and most Kubernetes pods: can escalate to host root.
...
Two severe Linux kernel LPEs in the same week, from the same bug class, both introduced by in-place optimization commits from 2017. Both found with AI-assisted tooling. Both deterministic and broadly exploitable with tiny Python scripts.
